...
namespace HospiceSource.Digital.NetSuite.SDK.OAuth
{
    ...
    public class OAuthRequest
    {
        ...
        public static string ToRequestValue(OAuthSignatureMethod signatureMethod)
        {
            ...
            {
                var sha1Index = value.IndexOf("SHA1");
                return sha1Index > -1 ? value.Insert(sha1Index, "-") : value;
            }
            var sha2Index = value.IndexOf("SHA256");
            return sha2Index > -1 ? value.Insert(sha2Index, "-") : value;
        }
